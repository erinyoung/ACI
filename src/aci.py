...
def main():
    """
    Main script for getting the depth of each amplicon in a bedfile using pysam.
    """
    ...
    for input in list(itertools.product(args.bam,regions)):
        if (args.single):
            pool.submit(aci_depth, df, input[0], input[1], True, args.out)
            logging.debug('Single bam file: ' + input[0])
            logging.debug('Region: ' + input[1])
            # aci_depth(df,input[0],input[1],True)
        else:
            pool.submit(aci_depth, df, input[0], input[1], False, args.out)
            # aci_depth(df,input[0],input[1],False)
    ...
    df.to_csv(args.out + '/aci_depth.csv', index=False)
    ...
    boxplot.figure.savefig(args.out + '/aci_depth.png', bbox_inches='tight')
    plt.close()

    logging.info('Depth for amplicons is saved in '  + args.out + '/aci_depth.csv')
    logging.info('An boxplot of these depths is at ' + args.out + '/aci_depth.png')
